...
namespace EduardoBotv2.Core.Modules.News.Services
{
    public class NewsService
    {
        ...
        public async Task GetNewsHeadlines(EduardoContext context, string source)
        {
            ...
            using (Stream responseStream = await NetworkHelper.GetStreamAsync($"https://newsapi.org/v1/articles?source={source}&sortBy=top&apiKey={credentials.NewsApiKey}"))
            using (StreamReader sr = new StreamReader(responseStream))
            {
                ...
                {
                    string shorten = await ShortenHelper.ShortenUrlAsync(credentials.GoogleShortenerApiKey, jHeadlines[i]["url"].ToString());
