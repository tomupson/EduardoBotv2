...
namespace EduardoBotv2.Core.Modules.Moderation.Services
{
    public class ModerationService : IEduardoService
    {
        ...
        {
            await context.Channel.SendMessageAsync($"{Format.Bold(context.User.Mention)} has banned {banUser.Mention}.\nReason: {reason}");
        ...
        {
            await context.Channel.SendMessageAsync($"{Format.Bold(context.User.Mention)} has kicked {kickUser.Mention}.\nReason: {reason}");
            // Add kick code here.
        }
    }
}