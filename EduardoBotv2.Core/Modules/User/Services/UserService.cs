...
namespace EduardoBotv2.Core.Modules.User.Services
{
    public class UserService
    {
        public async Task DisplayUserInfo(EduardoContext context, IGuildUser user = null)
        {
            ...
            string userInfo = $"Created: {targetSocketUser?.CreatedAt:dddd MMM d}{DateTime.Now.Day.GetDaySuffix()} {targetSocketUser?.CreatedAt:yyyy} at {targetSocketUser?.CreatedAt:h:m tt}\n" +
                           $"Status: {targetSocketUser?.Status}\n" +
                           $"Game: {targetSocketUserGame}\n";

            string memberInfo = $"Joined: {targetGuildUser?.JoinedAt:dddd MMM d}{DateTime.Now.Day.GetDaySuffix()} {targetGuildUser?.JoinedAt:yyyy} at {targetGuildUser?.JoinedAt:h:m tt}\n" +
            ...
            EmbedBuilder builder = new EmbedBuilder
            {
                ...
                Footer = new EmbedFooterBuilder
                {
                    ...
                    Text = $"Eduardo | {string.Format("{0:dddd MMM d}{1} {0:yyyy} at {0:h:m tt}", DateTime.Now, DateTime.Now.Day.GetDaySuffix())}"
                },
